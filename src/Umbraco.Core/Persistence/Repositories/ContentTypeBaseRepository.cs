...
namespace Umbraco.Core.Persistence.Repositories
{
        ...
        where TEntity : class, IContentTypeComposition
    {
        ...
        public IEnumerable<MoveEventInfo<TEntity>> Move(TEntity moving, EntityContainer container)
        {
            ...
            var origPath = moving.Path;

            // move to new parent
            moving.ParentId = parentId;

            //set the updated path
            moving.Path = string.Concat(container == null ? parentId.ToInvariantString() : container.Path, ",", moving.Id);

            AddOrUpdate(moving);

            //update all descendants, update in order of level
            var descendants = GetByQuery(new Query<TEntity>().Where(x => x.Path.StartsWith(origPath + ",")));

            var lastParent = moving;
            foreach (var descendant in descendants.OrderBy(x => x.Level))
            {
                ...
                // FIXME THIS IS THE CURRENT 7.4 AND IT IS BROKEN
