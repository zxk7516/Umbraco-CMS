...
namespace Umbraco.Tests.Cache
{
    ...
    public class RefreshersTests
    {
        ...
        {
            var source = new[] { new ContentTypeCacheRefresher.JsonPayload("xxx", 1234, ContentTypeServiceBase.ChangeTypes.None) };
            var json = JsonConvert.SerializeObject(source);
        ...
        public void DataTypeCacheRefresherCanDeserializeJsonPayload()
        {
            ...
            Assert.AreEqual(source[0].Id, payload[0].Id);
            Assert.AreEqual(source[0].UniqueId, payload[0].UniqueId);
