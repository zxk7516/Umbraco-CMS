...
function umbTreeDirective($compile, $log, $q, $rootScope, treeService, notificationsService, $timeout, userService) {
    ...
    return {
        ...
        template: '<ul class="umb-tree"><li class="root">' +
                    '<div ng-hide="hideheader">' +
                    '<h5>' +
                    '<i ng-if="enablecheckboxes == \'true\'" ng-class="selectEnabledNodeClass(tree.root)"></i>'+
                    '<a href="#/{{section}}" ng-click="select(this, tree.root, $event)" on-right-click="altSelect(this, tree.root, $event)"  class="root-link">{{tree.name}}</a></h5>' +
